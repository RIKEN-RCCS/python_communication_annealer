				...
				while t != s:
					...
					for j in range(SIZE):
						if (A[j][t] == 1) and (d[j]+1 == d[t]):
							if ((f == 0) and (abs(perm_inv[j]-perm_inv[t]) == self.M)) or ((f == 1) and (abs(perm_inv[j]-perm_inv[t]) == 1)):
								next = min(next,(0,j))
							else:
								next = min(next,(1,j))
					if next[0] == np.inf:
...
if __name__ == "__main__":
	...
	main(lambda:Mesh2Dmodel(N,M,filename),SIZE,steps,filename,C)
